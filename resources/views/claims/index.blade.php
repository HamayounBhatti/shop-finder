...
<section class="app-user-list">
  ...
  <div class="card mt-4">
    ...
    <h1 class="mb-0 ml-3">Shop Claims</h1>
    ...
    <a href="claim/create" class="btn btn-primary mr-2">Add Claim</a>
</div>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li><a class="dropdown-item" href="{{route('claim.edit',$claim->id)}}">Edit</a></li>
                                    <li><a class="dropdown-item" href="{{route('claim.createSubmitRecord',$claim->id)}}">Submit</a></li>
									<li><a class="dropdown-item" href="{{route('claim.locallyClosed',$claim->id)}}">Closed</a></li>
            ...
            success: function(claim) {
                ...
                let tableBody = `
                    <tr>
                        ...
                        <td>${claim.cell}</td>
                        <td>${claim.shops.name}</td>
                        <td>${ claim.status == 0 ? 'Pending' :(claim.status == 1 ? 'Submitted':(claim.status == 2 ? 'Rejected' : (claim.status == 3 ? 'Repair':(claim.status ==4 ? 'Replacement':(claim.status == 5 ? 'Received': ''))))) }</td>
                        <td>${claim.is_closed == 0 ? 'No' : 'Yes'}</td>
						<td>
                            <div class="dropdown">
                                <button class="btn btn-secondary dropdown-toggle btn-sm" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                ...
                                <ul class="dropdown-menu">
								    <li><a class="dropdown-item" href="/claim/edit/${claim.id}">Edit</a></li>
                                    <li><a class="dropdown-item" href="/claim/createSubmitRecord',$claim->id)}}">Submit</a></li>
									<li><a class="dropdown-item" href="/claim/locally/closed',$claim->id)}}">Closed</a></li>
